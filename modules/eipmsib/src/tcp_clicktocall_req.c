...
int tcp_clicktocall_start_req_decode_msg( tcp_clicktocall_start_req_t *clicktocall_start_req, tcp_msg_t *msg)
{
	...
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->subscirberName = uxc_dbif_get_str( dbif, 2, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	...
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->hostingCode = uxc_dbif_get_int( dbif, 13, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->scenarioType = uxc_dbif_get_int( dbif, 14, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->recordingFileName = uxc_dbif_get_str( dbif, 15, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
