...
int tcp_clicktocall_start_req_decode_msg( tcp_clicktocall_start_req_t *clicktocall_start_req, tcp_msg_t *msg)
{
	...
	clicktocall_start_req->sessionID = uxc_dbif_get_str( dbif, 0, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->gwSessionID = uxc_dbif_get_str( dbif, 1, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->subscriberName = uxc_dbif_get_str( dbif, 2, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->recordingType = uxc_dbif_get_int( dbif, 3, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->callingNumber = uxc_dbif_get_str( dbif, 4, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->calledNumber = uxc_dbif_get_str( dbif, 5, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	...
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->scenarioType = uxc_dbif_get_int( dbif, 10, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->callMentID = uxc_dbif_get_int( dbif, 11, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->callingCID = uxc_dbif_get_str( dbif, 12, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->calledCID = uxc_dbif_get_str( dbif, 13, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->recordingFileName = uxc_dbif_get_str( dbif, 14, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->isAllRecording = uxc_dbif_get_int( dbif, 15, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->endIfRecordingFailed = uxc_dbif_get_int( dbif, 16, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->endIfRecordingEnded = uxc_dbif_get_int( dbif, 17, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->hostingCode = uxc_dbif_get_int( dbif, 18, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
	clicktocall_start_req->wirelessTimeout = uxc_dbif_get_int( dbif, 19, &rv);
	if( rv < eUXC_SUCCESS ) goto final;
