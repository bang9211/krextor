...
#define DBIF_RECORDING_START_REQUEST						    300
#define DBIF_RECORDING_STOP_REQUEST						        301

#define DBIF_RECORDING_START_RESPONSE						    400
#define DBIF_RECORDING_STOP_RESPONSE						    401
...
// v0.4에서 참여자 리스트의 최대 크기는 32명으로 제한한다. 추가로 회의에 참여시킬 참여자가 있는 경우는, 
// 회의가 생성되고 주관자가 회의에 참여하여 완전히 회의가 개시된 이후에 참여자 추가 요청을 사용한다.
// 참여자 간의 구분은 ’|’ 문자를 사용한다.
// 참여자 정보는 이름, 전화번호로 이루어 지며, 이 같은 참여자 정보 간의 구분은 ‘^’ 문자를 사용한다. 
// 전화 번호는 유선 전화 번호와 무선 전화번호 중 하나의 정보를 반드시 입력한다. 
// 리스트 중 마지막 참여자의 정보 뒤에는 Null Termination 처리한다.
