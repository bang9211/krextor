...
/**
 * @brief DBIF 메시지를 받아 TCP Server 에 전송한다.
 */
int tcp_handle_clireq(uxc_action_t *action, uxc_worker_t* worker, uxc_msg_t *msg)
{
	uxc_ipcmsg_t *ipcmsg;
	tcp_client_t *client;
...
/**
 * @brief TCP Server 로부터 메시지를 받아 DBIF 에 전송한다. 
 */
int tcp_handle_clirsp(uxc_action_t *action, uxc_worker_t* worker, uxc_msg_t *uxcmsg)
{
	int rv;
	tcp_client_t *client;
	upa_tcpmsg_t *tcpmsg;
...
/**
 * @brief TCP Client로 부ㅌ터 TCP DBIF msg 를 받아 decode/encode 후 ipcmsg 를 uxcutor 로 전송한다. 
 * @param action action
 * @param worker worker
